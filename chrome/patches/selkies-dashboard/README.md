...
The current series starts with a refreshed integration baseline, then accepts small incremental patches. It is applied by `scripts/sync-selkies-dashboard.sh` after copying upstream `addons/selkies-dashboard`. `0010-trackpad-portrait-default.patch` shows the Trackpad tile in portrait only and defaults it on for touch clients. `0013-mobile-speaker-default-off.patch` defaults the speaker toggle off on touch/mobile clients.
