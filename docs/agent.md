...
## Image clipboard and pending Paste batch (local candidate)

Image writes use `{t:"clipboard",id,mime:"image/png",data:base64}`. Preserve the correlation ID through agent helpers. The native receiver bounds decoded bytes to 768000, either dimension to 4096, and pixels to 8388608, and confirms both registered PNG and DIBV5 clipboard representations before reporting applied. The viewer checks its final 1 MiB envelope budget and does not replay writes after reconnect. Native read failures use `clipboard_result` with `id:null,direction:"read",sequence,status:"rejected",reason`; they never acknowledge a pending write.

Incoming PNG images are bounded before forwarding to the dashboard. The dashboard serializes text and image browser clipboard writes, retains only the latest blocked content for a trusted remote-screen gesture, and reports actual success or browser failure through existing notifications. Disconnect, Image Support OFF, a new text clipboard, and local clipboard edits discard pending delivery. A submitted browser clipboard write is not cancellable; reset suppresses its stale feedback and subsequent queued writes, not an already submitted OS operation. Browser fixture tests substitute the clipboard API and do not establish actual device clipboard acceptance.

The compatible native batch is implementing explicit pending-Paste support. A successful clipboard write may return a single-use `paste_token`, bound to its request ID and native clipboard sequence. Only a real pending plain Paste gesture may produce `{t:"clipboard_paste",id,clipboard_id,paste_token}` after confirmation and shortcut-key release. There is no implicit paste after Upload Image. Results use `clipboard_paste_result` with `injected`, `rejected`, or `uncertain`; injected means the shortcut events were accepted, not that an arbitrary application consumed the clipboard. The token is invalidated by replacement/toggle/session changes and consumed before dispatch, with no automatic retries. Native clipboard/target freshness checks reduce races; generic Windows keyboard injection does not make later application clipboard reads atomic. Browser intent/release/cancellation handling has controlled tests; real trusted-key and installed acceptance remain unfinished. Direct local-clipboard reading on Paste/focus and source image formats beyond PNG/JPEG/WebP remain parity work; this batch does not claim completion of all original clipboard behavior.

Static WebP input is additionally preflighted before browser decode: RIFF/chunk bounds, VP8/VP8L dimensions, extended-canvas agreement and animation rejection. It is converted to PNG without resizing. Google WebP container and lossless-bitstream specifications define these fields. A synthetic browser fixture decoded PNG/JPEG/WebP inputs and compared every resulting PNG RGBA sample against each decoded source: all32x32 cases had maximum error0. This proves those synthetic conversion cases, not every codec/profile or real clipboard transfer.
