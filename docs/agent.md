...
Local validation:108Worker tests,31viewer/protocol tests and TypeScript pass. Windows-native320×240 color bars decoded in Edge through this consumer for8frames and another8after a generation change; maximum sampled RGB error2, zero unexpected fallback. Invalid width321 requests JPEG once. These synthetic checks do not establish full-desktop performance, resize acceptance or a deployable native release. Keep JPEG as the current default until integrated acceptance; verified H.264 as preferred default remains the target.
