...
**Mobile keyboard viewport:** When the OS keyboard opens on touch clients, the parent shell (`public/index.html`) shrinks `#jolee-core` into `visualViewport` so Trackpad Mode still works in the visible area above the keyboard. Layout restores when the keyboard dismisses. Feature flag default **on**; disable with `?kbd_shift=0`. Standalone `viewer.html` does the same when not embedded.
