...
**Mobile keyboard viewport:** Portrait touch only. Pins `#jolee-core` to `visualViewport` (exact visible area above the OSK). Trackpad touches keep the parent keyboard assist focused so the OSK stays up. Landscape unchanged. Flag default **on**; `?kbd_shift=0` disables.
