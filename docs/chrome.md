...
**Mobile keyboard viewport (parked — currently broken):** Active development; **currently broken** on real-phone OSK (confirmed 2026-09-13). Parked — do **not** enable by default. Flag stays default **off**; opt in only with `?kbd_shift=1` on the join URL (force off with `=0`). Tries to pin `#jolee-core` to `visualViewport` above the OSK and use trackpad `streamGesture` so the shell reclaims keyboard-assist focus / skips pin reflow mid-drag.
