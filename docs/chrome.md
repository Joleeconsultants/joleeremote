...
**Mobile keyboard viewport:** Portrait touch only. Pins `#jolee-core` to `visualViewport` above the OSK; trackpad keeps parent keyboard assist focused. Landscape unchanged. Flag default **off**; enable with `?kbd_shift=1` (force off with `=0`).
