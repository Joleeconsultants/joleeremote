...
| `pipelineControl` | `{pipeline, enabled}` | `video`: gate canvas paint + status; `audio`: mute/stop playback + status; `microphone`/`webcam`: capture start/stop (`{t:mic}` / `{t:webcam}`). Dashboard primes mic permission in the top window on Enable click; viewer then captures (preferred MediaRecorder mimes then bare; deviceId falls back to audio:true). Failures post `pipelineStatusUpdate` with `error` (no silent no-op). HTML shells advertise `Permissions-Policy: microphone=*, camera=*, fullscreen=*` and the core iframe allows `microphone *` / `camera *`. Also input `{t:pipeline, pipeline, enabled}` |
