...
**Mobile keyboard viewport:** Portrait touch only. When the OS keyboard opens, the parent shell elevates `#jolee-core` with `translateY(-keyboardHeight)` (keeps full size — does not shrink) so Trackpad Mode works in the area above the keyboard. Landscape leaves full layout. Restores when the keyboard dismisses or orientation flips to landscape. Flag default **on**; `?kbd_shift=0` disables. Standalone `viewer.html` honors the same flag when not embedded.
