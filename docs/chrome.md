...
**Mobile keyboard viewport:** Portrait touch only. When the OS keyboard opens, the parent shell (`public/index.html`) shrinks `#jolee-core` into `visualViewport` so Trackpad Mode still works in the visible area above the keyboard. Landscape leaves full layout (shift does not apply). Layout restores when the keyboard dismisses or orientation flips to landscape. One feature flag (default **on**): `?kbd_shift=0` turns off viewport shift, the trackpad no-focus-steal, and gesture reflow pause together — easy full reverse. Standalone `viewer.html` honors the same flag when not embedded.

**Mobile speaker:** On touch/coarse-pointer clients the speaker (audio playback) toggle starts **off**. Desktop stays on. No localStorage — session default only. Viewer (`pointer: coarse`) and Sidebar (`isMobileClient`) share that default.
