...
**Mobile keyboard viewport (active development):** Portrait touch only. Pins `#jolee-core` to `visualViewport` above the OSK; trackpad posts `streamGesture` so the shell reclaims keyboard-assist focus and skips pin reflow mid-drag. Landscape unchanged. **Not ready for default use** — flag default **off**; opt in with `?kbd_shift=1` on the join URL (force off with `=0`). Expect breakage until this note is removed.
