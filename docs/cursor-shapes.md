...
Both existing cursor modes consume the same PC update: drawn mode renders its PNG; native/CSS mode uses a validated semantic stock cursor type from the local OS. Missing or unknown semantic types fall back to the local arrow, never a remote bitmap. A hidden update hides both. A new hotspot repositions a stationary overlay. Session loss clears the old shape. The sidebar layout and toggle remain unchanged.
...
Native shape recognition currently compares the Windows cursor handle against shared stock handles. Unmatched application-specific or copied handles can omit the semantic type. Kitchen is under live investigation; the local arrow fallback does not establish why its handle was unmatched.
